...
onload = function() {
  ...
  /***** 
  *********
  Get User Input and test if it starts with http/https/www 
  *****/ 
  document.querySelector('#location-form').onsubmit = function(e) {
    e.preventDefault();
    var location = document.querySelector('#location').value;
    var regex = /^((http?|ftp|smtp):\/\/)?(www.)?[a-z0-9]+(\.[a-z]{2,}){1,3}(#?\/?[a-zA-Z0-9#]+)*\/?(\?[a-zA-Z0-9-_]+=[a-zA-Z0-9-%]+&?)?$/;
    var testHttp = regex.test(location);

    if (testHttp == true) {
        navigateTo(location);
    }
    else{
      navigateTo(`https://www.google.com/search?q=${location}&oq=${location}&sourceid=chrome&ie=UTF-8`);      
    }
  ...
  *****/ 
  ...
  //Add icon class 
  document.querySelector("#back").classList.add('icon-arrow-left');
  document.querySelector("#forward").classList.add('icon-arrow-right');
  document.querySelector("#home").classList.add('icon-home');
  document.querySelector("#reload").classList.add('icon-reload');
  document.querySelector("#favouriteAdd").classList.add('icon-heart','text-danger');
  document.querySelector("#searchIcon").classList.add('icon-magnifier');

  const addOnIcon = document.querySelector("#addons");
  addOnIcon.classList.add('icon-options-vertical');

  $("#addons").click(function () {
    webview.openDevTools();
  });

  const menuIcon = document.querySelector("#preference");
  menuIcon.classList.add('icon-menu');




  //Add to favouurite and store in local storage
  var favourites = [];
  $("#favouriteAdd").click(function () {
      var currentPage = webview.getURL();
      favourites.push(currentPage);
      localStorage.favourites += favourites;
      console.log(currentPage + "was added to favourites");
